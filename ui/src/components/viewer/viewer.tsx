  ...
  clips
    .filter(
      clip =>
        ...
        clip.timePosition <= givenTime &&
        clip.timePosition + clip.duration > givenTime
    )
    .map(clip => clip.id);
...
const ViewerProgress = ({ coord, currentClip }: ViewerProgressProps) => {
  ...
  const onSeek = (intent: number) => {
    ...
    if (
      curClipRelTimePosition < currentClip.duration &&
      curClipRelTimePosition > 0
    ) {
      player.seekTo(curClipRelTimePosition);
    }
  };

  return <ProgressBar length={coord.length} />;
};
...
const ViewerTimeline = ({
  ...
  React.useEffect(() => {
    ...
    if (
      ...
      const nextClipId: number | undefined = getPlayableClipIds(
        coord.clips,
        playbackTime
      )[0];
...
export const Viewer = ({ coord }: ViewerProps) => {
  ...
  return (
    <ViewerContainer>
      <PlayerProvider coord={coord}>
        <Player clip={currentClip} startTime={startTime} />

        <ViewerProgress coord={coord} currentClip={currentClip} />
