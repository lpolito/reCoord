...
export const PlayerProvider = ({
    ...
    const updatePlayer = React.useCallback(({id, clip, startTime}: Player) => (
    ...
    const seekPlayers = React.useCallback((newPlaybackTime: number) => {
        players.forEach((player) => {
            const curClipRelTimePositionA = (newPlaybackTime - player.clip.timePosition);

            // Seek player if curClipRelTimePosition falls within the current clip.
            if (curClipRelTimePositionA < player.clip.duration && curClipRelTimePositionA > 0) {
     ...
     */
    const onSeekHandle = React.useCallback((intent: number) => {
        ...
        if (isPlaying) {
            ...
            players.forEach((player) => {
                updatePlayer({
                    ...player,
                    startTime: calculateStartTime(newPlaybackTime, player.clip.timePosition),
                });
            });
        }
