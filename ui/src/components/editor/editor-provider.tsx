...
const calcTimelineInfo = (clips: Clip[]) =>
  clips.reduce(
    (acc: TimelineInfo, clip: Clip): TimelineInfo => {
      const coordEnd = clip.timePosition + clip.duration;
      // Earliest starting point of coord.
      const end =
        acc.end === undefined || coordEnd > acc.end ? coordEnd : acc.end;

      // Latest ending point of coord.
      const start =
        acc.start === undefined || clip.timePosition < acc.start
          ? clip.timePosition
          : acc.start;
...
const coordReducer = (
  ...
  switch (type) {
    ...
    case 'updateClip': {
      const clips = prevCoord.clips.map(
        (clip): Clip => {
...
export const EditorProvider = ({
  ...
  const { coord, updateCoord, updateClip } = useCoordEditor(initialCoord);
